...
#脚本在 ROS 环境中运行：订阅相机、机械臂、里程计等话题；构建并加载已训练的 policy（ACT/CNNMLP/Diffusion）；
# 在实时循环里用最新同步到的一帧（多相机 + 关节）作为 observation 做一次前向推理，
# 得到动作并发布到 puppet（伪装/仿真）机械臂与底盘上，实现在线推理控制。
