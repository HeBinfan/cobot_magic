...
# train.py 是训练脚本：读取数据集、构造 policy（ACT/CNNMLP/Diffusion 之一）、训练与验证循环、
# 保存最优 checkpoint，并绘制训练/验证曲线。核心训练逻辑在 train_process() 中。

# 训练函数，负责整体训练流程的管理和执行。包括初始化配置、数据加载、启动训练循环
def train(args):
    set_seed(1) #设置随机种子，确保实验的可重复性。

    DATA_DIR = os.path.expanduser(args.dataset_dir) #数据集目录路径，展开用户目录符号（~）。

    # 定义任务配置字典，包含数据集路径、摄像头名称和训练集大小等信息。
    TASK_CONFIGS = {
        args.task_name: {
            'dataset_dir': os.path.join(DATA_DIR, args.task_name),  #数据集目录路径
            'camera_names': ['cam_high', 'cam_left_wrist', 'cam_right_wrist'],  #使用的摄像头名称列表
            'num_episodes': args.num_episodes   #训练集中的总任务数
        }
    }

    task_config = TASK_CONFIGS[args.task_name]  #根据任务名称获取对应的任务配置。
    ...
    # 配置策略参数
    # 根据策略类型（ACT/CNNMLP/Diffusion）构建对应的policy_config（封装学习率、网络结构、损失函数、策略专属超参数等）；
    # fixed parameters
    if args.policy_class == 'ACT':
        ...
        raise NotImplementedError   #不支持的策略类型，抛出异常。

    # 训练配置字典，包含训练周期数、checkpoint目录、策略类型与配置、随机种子等信息。
    config = {
    ...
    # 调用load_data加载训练 / 验证数据，保存数据集统计信息（stats，用于数据归一化 / 反归一化）
    # data Preprocess
    ...
    # 如果 checkpoint 目录不存在，则创建该目录；然后将数据集统计信息保存为 pickle 文件，供后续使用。
    # save dataset stats
...
# 训练过程函数，执行训练和验证循环，保存最优模型 checkpoint，并绘制训练/验证曲线。
def train_process(train_dataloader, val_dataloader, config, stats):
    ...
    policy_config = config['policy_config'] # policy参数
...
def get_arguments():
    ...
    parser.add_argument('--pretrain_ckpt', action='store', type=str, help='pretrain_ckpt', default='', required=False)  #预训练模型路径，默认为空字符串，使用预训练会不会更好？
...
def main():
    args = get_arguments()  #调用get_arguments()解析命令行参数；
    train(args) #将解析后的参数传入train()函数，启动训练核心逻辑。
